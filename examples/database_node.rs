...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    transport.listen("127.0.0.1:8080".to_string(), false).await?;
    node.start().await?;
    ...
    // Add contacts
    node.add_contact("alice".to_string(), "alice_pubkey_123".to_string()).await?;
    node.add_contact("bob".to_string(), "bob_pubkey_456".to_string()).await?;
    
    // Add channels
    node.add_channel("general".to_string(), "general_channel_privkey".to_string()).await?;
    node.add_channel("private".to_string(), "private_channel_privkey".to_string()).await?;
    
    // Add profiles
    node.add_profile("default_profile".to_string(), true).await?;
    
    // Add peers
    node.add_peer("peer1".to_string(), true, "127.0.0.1:8081".to_string(), Some("main".to_string())).await?;
    node.add_peer("peer2".to_string(), false, "127.0.0.1:8082".to_string(), Some("backup".to_string())).await?;
