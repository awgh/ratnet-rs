...
use bytes::Bytes;
use ratnet::api::{Msg, PubKey, Node, Transport};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Sender transport byte limit: {}", sender_transport.byte_limit());
    info!("Receiver transport byte limit: {}", receiver_transport.byte_limit());
    ...
    info!("Sending large message: {} bytes", large_message.content.len());
    info!("With transport limit: {} bytes, this will require chunking", sender_transport.byte_limit());

    // Calculate expected chunks
    let chunk_size = ratnet::api::chunking::calculate_chunk_size(sender_node.clone()).await.unwrap_or(4096);
    ...
    info!("Sending small message: {} bytes", small_message.content.len());
    ...
    info!("Force chunking {} byte message into {} byte chunks", 
          manual_message.content.len(), custom_chunk_size);
    
    ratnet::api::chunking::send_chunked(
        sender_node.clone(),
        custom_chunk_size,
        manual_message
    ).await?;
