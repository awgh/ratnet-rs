...
use ratnet::prelude::*;
use ratnet::{nodes::MemoryNode, router::DefaultRouter, transports::UdpTransport, policy::PollPolicy};
use std::sync::Arc;
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    // Add some test data
    node.add_contact("test_contact".to_string(), "test_pubkey_123".to_string()).await?;
    node.add_peer("test_peer".to_string(), true, "127.0.0.1:8081".to_string(), Some("default".to_string())).await?;
