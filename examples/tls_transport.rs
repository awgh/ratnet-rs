...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    transport.listen("127.0.0.1:8443".to_string(), false).await?;
    node.start().await?;
    ...
    // Add some test data
    node.add_contact("alice".to_string(), "alice_tls_pubkey".to_string()).await?;
    node.add_peer("tls_peer".to_string(), true, "127.0.0.1:8444".to_string(), Some("secure".to_string())).await?;
