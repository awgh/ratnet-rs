use ratnet::api::crypto::{KeyPair, PubKey, generate_hybrid_keypair, encrypt, decrypt};
...
async fn demo_ed25519_functionality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Signature verification: {}", if is_valid { "SUCCESS" } else { "FAILED" });
    
    // Test signature verification with wrong data
    let wrong_message = b"Wrong message!";
    let is_valid_wrong = KeyPair::verify(&pubkey, wrong_message, &signature)?;
    info!("Wrong message verification: {}", if is_valid_wrong { "SUCCESS (UNEXPECTED)" } else { "FAILED (EXPECTED)" });
...
async fn demo_hybrid_keypairs() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Ed25519 signature verification: {}", if is_valid { "✅ SUCCESS" } else { "❌ FAILED" });
    ...
    info!("Kyber encryption/decryption: {}", if encryption_success { "✅ SUCCESS" } else { "❌ FAILED" });
...
async fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Ed25519 Signing ({} iterations): {:?}", iterations, ed25519_sign_time);
    info!("Ed25519 Average signing time: {:?}", ed25519_sign_time / iterations);
    
    info!("Kyber Key Generation: {:?}", kyber_keygen_time);
    info!("Kyber Encryption/Decryption ({} iterations): {:?}", iterations, kyber_encrypt_time);
    info!("Kyber Average encryption/decryption time: {:?}", kyber_encrypt_time / iterations);
...
async fn demo_key_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Ed25519 Serialization: {}", if ed25519_pubkey == ed25519_deserialized { "✅ SUCCESS" } else { "❌ FAILED" });
    ...
    info!("Kyber Serialization: {}", if kyber_pubkey == kyber_deserialized { "✅ SUCCESS" } else { "❌ FAILED" });
    
    // Test nil key
    let nil_key = PubKey::nil();
    let nil_serialized = nil_key.to_string();
    let nil_deserialized = PubKey::from_string(&nil_serialized)?;
    info!("Nil key serialization: {}", if nil_key == nil_deserialized { "✅ SUCCESS" } else { "❌ FAILED" });
