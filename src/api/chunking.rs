...
pub async fn calculate_chunk_size(node: Arc<dyn Node>) -> Result<u32> {
    ...
    if chunk_size <= CHUNK_OVERHEAD + CHUNK_HEADER_SIZE {
        return Err(RatNetError::InvalidArgument(
            format!("Transport byte limit {} too small for chunking", chunk_size)
        ));
    }
...
pub async fn send_chunked(
    ...
    if chunk_payload_size == 0 {
        return Err(RatNetError::InvalidArgument("Chunk size too small".to_string()));
    }
    
    let whole_chunks = content_len / chunk_payload_size;
    let remainder = content_len % chunk_payload_size;
    let total_chunks = if remainder > 0 { whole_chunks + 1 } else { whole_chunks };
    ...
    debug!("Chunking message: {} bytes into {} chunks", content_len, total_chunks);
...
async fn send_stream_header(
    ...
    trace!("Sending stream header: stream_id={:08x}, total_chunks={}", stream_id, total_chunks);
...
async fn send_chunk(
    ...
    trace!("Sending chunk: stream_id={:08x}, chunk_num={}, size={}", 
           stream_id, chunk_num, data.len());
...
async fn handle_stream_header(node: Arc<dyn Node>, msg: Msg) -> Result<()> {
    ...
    debug!("Received stream header: stream_id={:08x}, total_chunks={}, channel='{}'", 
...
async fn handle_chunk(node: Arc<dyn Node>, msg: Msg) -> Result<()> {
    ...
    debug!("Received chunk: stream_id={:08x}, chunk_num={}, size={}", 
