...
use pqcrypto_kyber::kyber1024::{
    keypair, encapsulate, decapsulate, PublicKey as KyberPublicKey, SecretKey as KyberSecretKey,
...
impl fmt::Display for PubKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PubKey::Ed25519(bytes) => write!(f, "ed25519:{}", base64::engine::general_purpose::STANDARD.encode(bytes)),
            PubKey::Kyber(bytes) => write!(f, "kyber:{}", base64::engine::general_purpose::STANDARD.encode(bytes)),
...
impl KeyPair {
    ...
    pub fn sign(&self, data: &[u8]) -> Result<Vec<u8>> {
        match self {
            ...
            KeyPair::Kyber { .. } => {
                Err(RatNetError::Crypto("Kyber keys cannot be used for signing".to_string()))
            }
        }
    }
    ...
    pub fn verify(pubkey: &PubKey, data: &[u8], signature: &[u8]) -> Result<bool> {
        match pubkey {
            ...
            PubKey::Kyber(_) => {
                Err(RatNetError::Crypto("Kyber keys cannot be used for signature verification".to_string()))
            }
            PubKey::Nil => Ok(false),
        }
    }
    ...
    pub fn from_string(s: &str) -> Result<Self> {
        ...
        if s.starts_with("ed25519:") {
            let encoded = &s[8..];
            let pkcs8_bytes = base64::engine::general_purpose::STANDARD.decode(encoded)
                .map_err(|e| RatNetError::Crypto(format!("Invalid Ed25519 key encoding: {}", e)))?;
            ...
            let encoded = &s[6..];
            let key_data = base64::engine::general_purpose::STANDARD.decode(encoded)
                .map_err(|e| RatNetError::Crypto(format!("Invalid Kyber key encoding: {}", e)))?;
            
            // Kyber key data should contain both public and secret keys
            // Format: [public_key_bytes][secret_key_bytes]
            if key_data.len() < 1568 + 3168 { // Kyber1024 public + secret key sizes
                return Err(RatNetError::Crypto("Invalid Kyber key data length".to_string()));
            }
...
pub fn encrypt(pubkey: &PubKey, data: &[u8]) -> Result<Vec<u8>> {
    match pubkey {
        PubKey::Ed25519(_) => {
            ...
            Err(RatNetError::Crypto("Ed25519 encryption not implemented".to_string()))
        }
        PubKey::Kyber(public_key_bytes) => {
            ...
            let final_key: Vec<u8> = symmetric_key.iter()
        ...
        PubKey::Nil => {
            Err(RatNetError::Crypto("Cannot encrypt with nil key".to_string()))
        }
    }
}
...
pub fn decrypt(keypair: &KeyPair, data: &[u8]) -> Result<Vec<u8>> {
    match keypair {
        KeyPair::Ed25519 { .. } => {
            Err(RatNetError::Crypto("Ed25519 decryption not implemented".to_string()))
        }
        KeyPair::Kyber { secret_key, .. } => {
            // Check minimum data size (ciphertext + key + encrypted data)
            if data.len() < 1568 + 32 + 16 { // Kyber1024 ciphertext + 32-byte key + minimum AES-GCM overhead
                return Err(RatNetError::Crypto("Invalid encrypted data size".to_string()));
            }
            ...
            let symmetric_key: Vec<u8> = xor_key.iter()
                .zip(shared_secret.as_bytes().iter())
                .map(|(a, b)| a ^ b)
                .collect();
...
fn encrypt_aes_gcm(key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    let mut sealing_key = SealingKey::new(unbound_key, IncrementingNonce { counter: AtomicU64::new(0) });
...
fn decrypt_aes_gcm(key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    if data.len() < 16 { // minimum tag size
    ...
    let unbound_key = ring::aead::UnboundKey::new(&AES_256_GCM, key)
        .map_err(|e| RatNetError::Crypto(format!("Invalid AES key: {:?}", e)))?;
    let mut opening_key = OpeningKey::new(unbound_key, IncrementingNonce { counter: AtomicU64::new(0) });
    
    // Decrypt the data
    let mut decrypted_data = data.to_vec();
    let decrypted_len = opening_key.open_in_place(Aad::empty(), &mut decrypted_data)
...
mod tests {
    ...
    fn test_keypair_serialization() {
        ...
        assert_eq!(ed25519_keypair.public_key(), deserialized_ed25519.public_key());
