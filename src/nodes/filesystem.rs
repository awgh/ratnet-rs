...
impl FilesystemNode {
                    ...
                    if let Ok(timestamp) = Self::hex_to_timestamp(filename) {
                        if timestamp > last_time {
                            ...
                            if max_bytes > 0 && proposed_size > max_bytes {
                                if bytes_read == 0 {
                                    return Err(RatNetError::InvalidArgument("Message too large for transport".to_string()));
                                }
                                break;
                            }
    ...
    async fn load_channel_messages(&self, channel_name: &str, last_time: i64, max_bytes: i64) -> Result<Vec<(i64, Vec<u8>)>> {
        let mut messages = Vec::new();
        let mut bytes_read = 0i64;
        ...
        if let Some(stream_info) = self.streams.get(&stream_id) {
            ...
            for i in 0..stream_info.total_chunks {
                if let Some(chunk) = self.chunks.get(&(stream_id, i)) {
                    ...
                    return Err(RatNetError::InvalidArgument(format!("Missing chunk {} in stream {}", i, stream_id)));
                }
            }
            Ok(complete_data)
        } else {
            Err(RatNetError::InvalidArgument(format!("Stream {} not found", stream_id)))
        }
    }
        ...
        if let Some(stream_info) = self.streams.get(&stream_id) {
            ...
            for chunk_num in 0..stream_info.total_chunks {
                if let Some(chunk_data) = self.chunks.get(&(stream_id, chunk_num)) {
                    ...
                    return Err(RatNetError::InvalidArgument(format!("Missing chunk {} in stream {}", chunk_num, stream_id)));
                }
            }
...
impl Node for FilesystemNode {
    async fn start(&self) -> Result<()> {
        if self.running.load(Ordering::Relaxed) {
            return Err(RatNetError::InvalidArgument("Node is already running".to_string()));
        }
    ...
    async fn stop(&self) -> Result<()> {
        if !self.running.load(Ordering::Relaxed) {
            return Err(RatNetError::InvalidArgument("Node is not running".to_string()));
        }
    ...
    async fn handle(&self, msg: Msg) -> Result<bool> {
        ...
        let decrypted_content = {
            ...
            let content_key = content_key_guard
                .as_ref()
        ...
        if let Some(stream_id_str) = message_data.get("stream_id").and_then(|v| v.as_str()) {
            ...
            let stream_id = stream_id_str.parse::<u32>()
                .map_err(|_| RatNetError::InvalidArgument("Invalid stream_id format".to_string()))?;
            
            let chunk_data = message_data.get("chunk_data").and_then(|v| v.as_str())
                .ok_or_else(|| RatNetError::InvalidArgument("Missing chunk_data in stream message".to_string()))?;
            let chunk_index = message_data.get("chunk_index").and_then(|v| v.as_u64())
                .ok_or_else(|| RatNetError::InvalidArgument("Missing chunk_index in stream message".to_string()))?;
            
            // Add chunk to stream
            self.add_chunk(stream_id, chunk_index as u32, chunk_data.as_bytes().to_vec().into()).await?;
    ...
    async fn forward(&self, msg: Msg) -> Result<()> {
        ...
        let channel_name = if msg.is_chan { Some(msg.name.as_str()) } else { None };
    ...
    async fn add_chunk(&self, stream_id: u32, chunk_num: u32, data: Bytes) -> Result<()> {
        if let Some(mut stream_info) = self.streams.get_mut(&stream_id) {
            ...
            return Err(RatNetError::InvalidArgument(format!("Stream {} not found", stream_id)));
        }
        Ok(())
    }
    ...
    async fn pickup(&self, routing_pub: PubKey, last_time: i64, max_bytes: i64, channel_names: Vec<String>) -> Result<Bundle> {
        ...
        async fn collect_messages_from_dir(dir: &PathBuf, file_times: &mut BTreeMap<i64, (PathBuf, Vec<u8>)>) -> Result<()> {
            let mut entries = fs::read_dir(dir).await?;
            while let Some(entry) = entries.next_entry().await? {
                let path = entry.path();
                if path.is_file() {
    ...
    async fn add_contact(&self, name: String, key: String) -> Result<()> {
        ...
        let contact = Contact { name: name.clone(), pubkey: key };
    ...
    async fn get_channels(&self) -> Result<Vec<Channel>> {
        Ok(self.channels.iter().map(|c| Channel {
            name: c.name.clone(),
            pubkey: c.pubkey.clone(),
        }).collect())
    }
    ...
    async fn get_profiles(&self) -> Result<Vec<Profile>> {
        Ok(self.profiles.iter().map(|p| Profile {
            name: p.name.clone(),
            enabled: p.enabled,
            pubkey: p.privkey.public_key().to_string(),
        }).collect())
    ...
    async fn get_peers(&self, group: Option<String>) -> Result<Vec<Peer>> {
        let peers: Vec<Peer> = self.peers.iter()
            .filter(|p| {
    ...
    async fn admin_rpc(&self, _transport: Arc<dyn Transport>, call: RemoteCall) -> Result<RemoteResponse> {
        ...
        match call.action {
            Action::CID => {
                match self.cid().await {
                    Ok(pubkey) => Ok(RemoteResponse::success(serde_json::json!(pubkey.to_string()))),
            ...
            _ => {
                Ok(RemoteResponse::error(format!("Unknown admin action: {:?}", call.action)))
            }
        }
    }
        ...
        match call.action {
            Action::ID => {
                match self.id().await {
                    Ok(pubkey) => {
                        if pubkey.is_nil() {
                            Ok(RemoteResponse::error("Node has no routing key set".to_string()))
                        } else {
                            Ok(RemoteResponse::success(serde_json::json!(pubkey.to_string())))
                        }
                    }
                    Err(e) => Ok(RemoteResponse::error(e.to_string())),
                }
            ...
            Action::Pickup => {
                ...
                match self.pickup(routing_pub, last_time, max_bytes, channel_names).await {
            ...
            Action::Dropoff => {
                ...
                match serde_json::from_value::<Bundle>(bundle_json.clone()) {
                    Ok(bundle) => {
                        match self.dropoff(bundle).await {
                            Ok(()) => Ok(RemoteResponse::success(serde_json::Value::Null)),
                            Err(e) => Ok(RemoteResponse::error(e.to_string())),
                        }
                    }
            ...
            _ => {
                Ok(RemoteResponse::error(format!("Unknown public action: {:?}", call.action)))
            }
        }
    }
} 