...
impl Node for MemoryNode {
    ...
    async fn add_stream(&self, stream_id: u32, total_chunks: u32, channel_name: String) -> Result<()> {
        debug!("Adding stream: id={}, chunks={}, channel={}", stream_id, total_chunks, channel_name);
    ...
    async fn add_chunk(&self, stream_id: u32, chunk_num: u32, data: Bytes) -> Result<()> {
        debug!("Adding chunk: stream={}, chunk={}, size={}", stream_id, chunk_num, data.len());
    ...
    async fn check_stream_complete(&self, stream_id: u32) -> Result<()> {
        ...
        debug!("Reassembled stream {} into {} bytes", stream_id, msg.content.len());
        ...
        match call.action {
            ...
            Action::SendMsg => {
                ...
                match serde_json::from_value::<Msg>(msg_json.clone()) {
                    ...
                    Err(e) => Ok(RemoteResponse::error(format!("Invalid message format: {}", e))),
                }
            }
            ...
            Action::IsRunning => {
                Ok(RemoteResponse::success(serde_json::json!(self.is_running())))
            }
        ...
        match call.action {
            ...
            Action::Pickup => {
                ...
                let channel_names: Vec<String> = match &call.args[3] {
                    ...
                    _ => return Ok(RemoteResponse::error("Invalid argument 4 (expected array)".to_string())),
                };
                
                // Parse routing public key
                let routing_pub = match PubKey::from_string(routing_pub_str) {
                    Ok(pk) => pk,
                    Err(_) => return Ok(RemoteResponse::error("Invalid routing public key".to_string())),
                };
    ...
    async fn flush_outbox(&self, max_age_seconds: i64) -> Result<()> {
        ...
        for mut entry in self.outbox.iter_mut() {
            entry.value_mut().retain(|msg| {
                current_time - msg.timestamp < max_age_seconds
            });
        }
    ...
    async fn pickup(&self, routing_pub: PubKey, last_time: i64, max_bytes: i64, channel_names: Vec<String>) -> Result<Bundle> {
        debug!("Pickup for key: {}, channels: {:?}, max_bytes: {}", routing_pub, channel_names, max_bytes);
        ...
        debug!("Pickup returning bundle with {} messages, {} bytes", messages.len(), bundle_data.len());
    ...
    async fn get_contacts(&self) -> Result<Vec<Contact>> {
        Ok(self.contacts.iter().map(|entry| entry.value().clone()).collect())
    }
    ...
    async fn get_channels(&self) -> Result<Vec<Channel>> {
        Ok(self.channels.iter().map(|entry| Channel {
            name: entry.name.clone(),
            pubkey: entry.pubkey.clone(),
        }).collect())
    }
    ...
    async fn get_profiles(&self) -> Result<Vec<Profile>> {
        Ok(self.profiles.iter().map(|entry| Profile {
            name: entry.name.clone(),
            enabled: entry.enabled,
            pubkey: entry.privkey.public_key().to_string(),
        }).collect())
    ...
    async fn send_msg(&self, msg: Msg) -> Result<()> {
        debug!("Sending message: name='{}', size={}, chunked={}", 
               msg.name, msg.content.len(), msg.chunked);
...
impl MemoryNode {
    ...
    fn send_msg_internal(&self, msg: Msg) -> Result<()> {
        ...
        let msg_data = match serde_json::to_vec(&msg) {
            ...
            Err(e) => {
                ...
                return Err(RatNetError::Serialization(format!("Failed to serialize message: {}", e)));
            }
        };
        
        let outbox_msg = OutboxMsg {
            channel: if msg.is_chan { Some(msg.name.clone()) } else { None },
            msg: msg_data,
        ...
        if let Some(channel) = &outbox_msg.channel {
            self.outbox.entry(channel.clone()).or_insert_with(Vec::new).push(outbox_msg);
        } else {
            self.outbox.entry(String::new()).or_insert_with(Vec::new).push(outbox_msg);
        }
        ...
        match serde_json::from_slice::<Vec<Msg>>(bundle_data) {
            ...
            Err(_) => {
                ...
                match serde_json::from_slice::<Msg>(bundle_data) {
                    ...
                    Err(e) => Err(RatNetError::Serialization(format!("Failed to deserialize bundle: {}", e))),
                }
            }
        }
    }
    ...
    async fn serialize_messages_to_bundle(&self, messages: &[Msg]) -> Result<Bytes> {
        ...
        let json_data = serde_json::to_vec(&messages)
            .map_err(|e| RatNetError::Serialization(format!("Failed to serialize messages: {}", e)))?;
    ...
    async fn deserialize_outbox_msg(&self, msg_data: &[u8]) -> Result<Msg> {
        serde_json::from_slice(msg_data)
            .map_err(|e| RatNetError::Serialization(format!("Failed to deserialize outbox message: {}", e)))
    }
