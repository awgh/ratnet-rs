...
impl DatabaseNode {
    ...
    async fn load_keys(&self) -> Result<()> {
        ...
        if let Some(_content_key) = self.database.get_config("contentkey").await? {
            ...
            self.database
                .set_config("contentkey".to_string(), content_key_str)
                .await?;
            self.database
                .set_config("routingkey".to_string(), routing_key_str)
                .await?;
    ...
    async fn save_outbox_msg(&self, msg: &Msg) -> Result<()> {
        ...
        self.database
            .add_outbox_msg(channel, &msg.content, timestamp)
            .await?;
        Ok(())
    }
    ...
    async fn sync_from_database(&self) -> Result<()> {
        ...
        for contact in contacts {
            self.memory_node
                .add_contact(contact.name, contact.pubkey)
                .await?;
        }
        ...
        for peer in peers {
            self.memory_node
                .add_peer(peer.name, peer.enabled, peer.uri, Some(peer.group))
                .await?;
        }
...
impl Node for DatabaseNode {
    ...
    async fn add_stream(
        ...
        // Save to database
        self.database
            .add_stream(stream_id, total_chunks, channel_name.clone())
            .await?;

        // Forward to memory node
        self.memory_node
            .add_stream(stream_id, total_chunks, channel_name)
            .await
    }
    ...
    async fn add_profile(&self, name: String, enabled: bool) -> Result<()> {
        ...
        self.database
            .add_profile(name, enabled, privkey_str)
            .await?;
        Ok(())
    }
