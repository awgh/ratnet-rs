...
pub type PolicyFactory = Box<
    dyn Fn(Arc<dyn Transport>, Arc<dyn Node>, HashMap<String, Value>) -> Arc<dyn Policy>
        + Send
        + Sync,
>;
...
pub type TransportFactory =
    Box<dyn Fn(Arc<dyn Node>, HashMap<String, Value>) -> Arc<dyn Transport> + Send + Sync>;
...
impl Registry {
    ...
    where
        F: Fn(Arc<dyn Transport>, Arc<dyn Node>, HashMap<String, Value>) -> Arc<dyn Policy>
            + Send
            + Sync
            + 'static,
    {
    ...
    pub fn new_router_from_map(&self, config: HashMap<String, Value>) -> Result<Arc<dyn Router>> {
        let router_type = config
            .get("Router")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                RatNetError::InvalidArgument("Missing or invalid Router type".to_string())
            })?;
    ...
    pub fn new_policy_from_map(
        ...
        let policy_type = config
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                RatNetError::InvalidArgument("Missing or invalid Policy type".to_string())
            })?;
    ...
    pub fn new_transport_from_map(
        ...
        let transport_type = config
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                RatNetError::InvalidArgument("Missing or invalid Transport type".to_string())
            })?;
