...
use crate::api::{Bundle, Node, PeerInfo, Transport, Action, RemoteCall, RemoteResponse};
...
impl PeerTable {
    ...
    pub async fn poll_server(
        ...
        if self.read_peer_table(host).await.is_none() {
            self.write_peer_table(host.to_string(), Arc::new(PeerInfo {
                ...
                Ok(response) => {
                    if let Ok(pubkey) = serde_json::from_value::<PubKey>(response) {
                        ...
                        let updated_peer = Arc::new(PeerInfo {
                            last_poll_local: AtomicI64::new(peer.last_poll_local.load(Ordering::Relaxed)),
                            last_poll_remote: AtomicI64::new(peer.last_poll_remote.load(Ordering::Relaxed)),
                            total_bytes_tx: AtomicI64::new(peer.total_bytes_tx.load(Ordering::Relaxed)),
                            total_bytes_rx: AtomicI64::new(peer.total_bytes_rx.load(Ordering::Relaxed)),
                            routing_pub: Some(pubkey),
                        });
                        self.write_peer_table(host.to_string(), updated_peer.clone()).await;
                        let peer = updated_peer;
                    } else {
                        error!("Failed to deserialize remote routing key");
                        return Err(RatNetError::Serialization("Invalid routing key format".to_string()));
                    }
                }
                Err(e) => {
        ...
        let to_local_raw = transport.rpc(
            host,
            Action::Pickup,
            vec![
        ...
        match to_local_raw {
            Ok(response) => {
                if let Ok(bundle) = serde_json::from_value::<Bundle>(response) {
                    ...
                    peer.total_bytes_rx.fetch_add(bundle.data.len() as i64, Ordering::Relaxed);
            ...
            match transport.rpc(
                ...
                Ok(_) => {
                    ...
                    if peer.total_bytes_tx.load(Ordering::Relaxed) > 0 {
                        peer.last_poll_local.store(to_remote.time, Ordering::Relaxed);
                    }
                    peer.total_bytes_tx.fetch_add(to_remote.data.len() as i64, Ordering::Relaxed);
                }
                Err(e) => {
