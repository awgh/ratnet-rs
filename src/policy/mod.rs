...
pub fn init() {
    ...
    crate::register_policy!("server", |transport, node, config| {
        let listen_uri = config.get("listen_uri")
            .and_then(|v| v.as_str())
            .unwrap_or("127.0.0.1:8080")
            .to_string();
        let admin_mode = config.get("admin_mode")
    ...
    crate::register_policy!("p2p", |transport, node, config| {
        ...
        let listen_interval = std::time::Duration::from_millis(
            config.get("listen_interval")
                .and_then(|v| v.as_u64())
                .unwrap_or(30000)
        );
        let advertise_interval = std::time::Duration::from_millis(
            config.get("advertise_interval")
                .and_then(|v| v.as_u64())
                .unwrap_or(10000)
        );

        Arc::new(p2p::P2PPolicy::new(
            transport, listen_uri, node, admin_mode, 
            listen_interval, advertise_interval
        ))
    });
} 