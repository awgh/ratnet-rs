...
impl CertificateBuilder {
    ...
    pub fn generate_ed25519_cert(&self) -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        params.not_after = (now + Duration::from_secs(self.validity_days as u64 * 24 * 60 * 60)).into();
...
pub fn validate_cert_chain(cert_chain: &[u8]) -> Result<bool> {
    ...
    if !cert_str.contains("-----BEGIN CERTIFICATE-----") {
        return Err(RatNetError::Crypto("Invalid certificate format".to_string()));
    }
...
pub fn extract_public_key_from_cert(cert_pem: &[u8]) -> Result<Vec<u8>> {
    ...
    if let Some(start) = cert_str.find("-----BEGIN CERTIFICATE-----") {
        if let Some(end) = cert_str.find("-----END CERTIFICATE-----") {
            ...
            let clean_content: String = cert_content.chars()
                .filter(|c| !c.is_whitespace())
                .collect();
            
            // Decode the base64 certificate
            let cert_der = base64::engine::general_purpose::STANDARD.decode(&clean_content)
            ...
            if cert_der.len() >= 32 {
                ...
                Err(RatNetError::Crypto("Invalid certificate structure".to_string()))
            }
        } else {
            Err(RatNetError::Crypto("Invalid certificate format".to_string()))
        }
    } else {
        Err(RatNetError::Crypto("Invalid certificate format".to_string()))
    }
