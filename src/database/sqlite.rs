...
use crate::api::*;
use crate::database::{Database, MigrationManager, default_migrations};
...
impl Database for SqliteDatabase {
    ...
    async fn get_peers(&self, group: Option<&str>) -> Result<Vec<Peer>> {
        let rows = if let Some(group) = group {
            sqlx::query("SELECT name, uri, enabled, peergroup FROM peers WHERE peergroup = ? ORDER BY name")
                .bind(group)
                .fetch_all(&self.pool)
                .await?
        } else {
    ...
    async fn add_peer(&self, name: String, enabled: bool, uri: String, group: String) -> Result<()> {
        sqlx::query("INSERT OR REPLACE INTO peers (name, uri, enabled, peergroup) VALUES (?, ?, ?, ?)")
            .bind(&name)
            .bind(&uri)
            .bind(enabled)
            .bind(&group)
    ...
    // Outbox operations
    async fn add_outbox_msg(&self, channel: Option<String>, msg: &[u8], timestamp: i64) -> Result<()> {
    ...
    async fn get_outbox_msgs(&self, since: i64) -> Result<Vec<OutboxMsg>> {
        let rows = sqlx::query("SELECT channel, msg, timestamp FROM outbox WHERE timestamp > ? ORDER BY timestamp")
            .bind(since)
            .fetch_all(&self.pool)
            .await?;
    ...
    async fn delete_outbox_msgs(&self, before: i64) -> Result<()> {
        ...
        debug!("Deleted {} outbox messages before timestamp {}", result.rows_affected(), before);
        Ok(())
    }
    ...
    async fn add_stream(&self, stream_id: u32, parts: u32, channel: String) -> Result<()> {
        ...
        debug!("Added stream: id={}, parts={}, channel={}", stream_id, parts, channel);
        Ok(())
    }
    ...
    async fn add_chunk(&self, stream_id: u32, chunk_num: u32, data: &[u8]) -> Result<()> {
        ...
        debug!("Added chunk: stream={}, chunk={}, size={}", stream_id, chunk_num, data.len());
        Ok(())
    }
    
    async fn get_chunk(&self, stream_id: u32, chunk_num: u32) -> Result<Option<Chunk>> {
        let row = sqlx::query("SELECT streamid, chunknum, data FROM chunks WHERE streamid = ? AND chunknum = ?")
            .bind(stream_id as i64)
            .bind(chunk_num as i64)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn get_stream_chunks(&self, stream_id: u32) -> Result<Vec<Chunk>> {
        let rows = sqlx::query("SELECT streamid, chunknum, data FROM chunks WHERE streamid = ? ORDER BY chunknum")
            .bind(stream_id as i64)
            .fetch_all(&self.pool)
            .await?;
