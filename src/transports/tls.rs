...
impl TlsTransport {
    ...
    async fn handle_connection(&self, mut stream: TlsStream<TcpStream>, admin_mode: bool) -> Result<()> {
    ...
    async fn read_buffer(&self, stream: &mut TlsStream<TcpStream>) -> Result<Vec<u8>> {
        ...
        if len > self.byte_limit.load(Ordering::Relaxed) as usize {
            return Err(RatNetError::Transport(format!("Message too large: {} bytes", len)));
        }
    ...
    async fn handle_tls_connection(
        ...
        Self::handle_connection_internal(tokio_rustls::TlsStream::Server(tls_stream), admin_mode).await
    }
...
impl Transport for TlsTransport {
    async fn listen(&self, listen: String, admin_mode: bool) -> Result<()> {
        if self.running.load(Ordering::Relaxed) {
            return Err(RatNetError::Transport("TLS transport is already running".to_string()));
        }
            ...
            while running.load(Ordering::Relaxed) {
                match listener.accept().await {
                    Ok((stream, addr)) => {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = TlsTransport::handle_tls_connection(stream, cert_pem, key_pem, admin_mode).await {
    ...
    async fn rpc(&self, host: &str, method: Action, args: Vec<serde_json::Value>) -> Result<serde_json::Value> {
        if !self.running.load(Ordering::Relaxed) {
            return Err(RatNetError::Transport("TLS transport not running".to_string()));
        }
...
impl JSON for TlsTransport {
    ...
    fn from_json(_s: &str) -> Result<Self> {
        ...
        Err(RatNetError::NotImplemented("TLS transport JSON deserialization not implemented".to_string()))
    }
}
