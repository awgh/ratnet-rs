...
impl BenchmarkRunner {
    ...
    pub async fn run_benchmark(&mut self) -> BenchmarkResult {
        ...
        let contact_keypair = crate::api::crypto::KeyPair::generate_ed25519().expect("Failed to generate contact key");
        let channel_keypair = crate::api::crypto::KeyPair::generate_ed25519().expect("Failed to generate channel key");
        ...
        let channel = Channel {
            ...
            pubkey: channel_keypair.to_string().expect("Failed to serialize channel key"),
        };
        
        // Add contact and channel
        node.add_contact(contact.name.clone(), contact.pubkey.clone()).await.expect("Failed to add contact");
        node.add_channel(channel.name.clone(), channel.pubkey.clone()).await.expect("Failed to add channel");
        ...
        for (i, message) in messages.iter().enumerate() {
            ...
            let msg = Msg {
                ...
                is_chan: true,
                pubkey: crate::api::crypto::PubKey::from_string(&channel.pubkey).unwrap_or(crate::api::crypto::PubKey::Nil),
                chunked: false,
                stream_header: false,
            };
            ...
            match node.send_msg(msg).await {
                Ok(_) => {
                    ...
                    if self.config.verbose {
                        println!("Sent message {}: {} bytes, latency: {:.2}ms", 
                                i, message.len(), latency.as_millis() as f64);
                    }
                }
                Err(e) => {
                    errors += 1;
        ...
        BenchmarkResult {
            ...
            min_latency_ms: if min_latency.is_infinite() { 0.0 } else { min_latency },
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!("  Message Size: {} bytes\n", self.config.message_size));
        report.push_str(&format!("  Concurrent Senders: {}\n", self.config.concurrent_senders));
        report.push_str(&format!("  Duration: {} seconds\n\n", self.config.duration_seconds));
        
        // Aggregate statistics
        let avg_throughput_mps: f64 = self.results.iter()
            .map(|r| r.throughput_mps)
            .sum::<f64>() / self.results.len() as f64;
            
        let avg_throughput_mbps: f64 = self.results.iter()
        ...
        let avg_cpu_usage: f64 = self.results.iter()
            .map(|r| r.cpu_usage_percent)
            .sum::<f64>() / self.results.len() as f64;
        
        report.push_str(&format!("Performance Summary:\n"));
        report.push_str(&format!("  Average Throughput: {:.2} messages/sec\n", avg_throughput_mps));
        report.push_str(&format!("  Average Throughput: {:.2} MB/sec\n", avg_throughput_mbps));
        ...
        for (i, result) in self.results.iter().enumerate() {
            ...
            report.push_str(&format!("  Throughput: {:.2} messages/sec\n", result.throughput_mps));
            report.push_str(&format!("  Latency (min/avg/max): {:.2}/{:.2}/{:.2} ms\n", 
    ...
    async fn create_node_and_transport(&self) -> (Arc<dyn Node>, Arc<dyn Transport>) {
        ...
        let transport: Arc<dyn Transport> = match self.config.transport_type {
            ...
            TransportType::Tls => {
                #[cfg(feature = "tls")]
                {
                    let (cert_pem, key_pem) = crate::crypto::generate_test_cert().expect("Failed to generate test certificate");
...
pub async fn comprehensive_benchmark() -> String {
    ...
    for result in all_results {
        report.push_str(&format!("{:?} + {:?}: {:.2} msg/sec, {:.2} MB/sec, {:.2}ms latency\n",
                               result.config.node_type, result.config.transport_type,
                               result.throughput_mps, result.throughput_mbps, result.avg_latency_ms));
    }
