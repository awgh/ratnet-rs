...
async fn test_udp_transport_echo() {
    ...
    let transport1 = Arc::new(ratnet::transports::UdpTransport::new("127.0.0.1:0".to_string()));
    let transport2 = Arc::new(ratnet::transports::UdpTransport::new("127.0.0.1:0".to_string()));
    
    // Start both transports
    transport1.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start transport1");
    transport2.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start transport2");
    
    // Get their addresses
    let addr1 = transport1.local_addr().await.expect("Failed to get transport1 address");
    let addr2 = transport2.local_addr().await.expect("Failed to get transport2 address");
    
    // Send a simple message from transport1 to transport2
    let test_data = b"Hello, UDP!";
    
    // Use the send_request method if available, otherwise use RPC
    let response = transport1.rpc(&addr2.to_string(), Action::ID, vec![serde_json::Value::String(String::from_utf8_lossy(test_data).to_string())]).await;
...
async fn test_udp_transport_rpc_communication() {
    ...
    let args1 = vec![serde_json::Value::String("Hello from transport1".to_string())];
    let args2 = vec![serde_json::Value::String("Hello from transport2".to_string())];
...
async fn test_udp_transport_timeout() {
    let transport = Arc::new(ratnet::transports::UdpTransport::new("127.0.0.1:0".to_string()));
    
    // Start transport
    transport.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start transport");
...
async fn test_udp_transport_restart() {
    ...
    for i in 0..5 {
        // Start
        transport.listen("127.0.0.1:0".to_string(), false).await.expect(&format!("Failed to start transport (iteration {})", i));
        ...
        // Stop
        transport.stop().await.expect(&format!("Failed to stop transport (iteration {})", i));
...
async fn test_udp_transport_different_ports() {
    ...
    let transport1 = Arc::new(ratnet::transports::UdpTransport::new("127.0.0.1:0".to_string()));
    let transport2 = Arc::new(ratnet::transports::UdpTransport::new("127.0.0.1:0".to_string()));
    let transport3 = Arc::new(ratnet::transports::UdpTransport::new("127.0.0.1:0".to_string()));
    
    // Start all transports
    transport1.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start transport1");
    transport2.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start transport2");
    transport3.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start transport3");
    
    // Get their addresses
    let addr1 = transport1.local_addr().await.expect("Failed to get transport1 address");
    let addr2 = transport2.local_addr().await.expect("Failed to get transport2 address");
    let addr3 = transport3.local_addr().await.expect("Failed to get transport3 address");
...
async fn test_udp_transport_admin_mode() {
    ...
    // Start in admin mode
    transport.listen("127.0.0.1:0".to_string(), true).await.expect("Failed to start transport in admin mode");
...
async fn test_tls_transport_basic() {
    ...
    let (cert_pem, key_pem) = ratnet::crypto::generate_test_cert()
    ...
    // Start transport
    transport.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start TLS transport");
    
    // Verify it's running
    assert!(transport.is_running());
    
    // Clean up
    transport.stop().await.expect("Failed to stop TLS transport");
}
...
async fn test_https_transport_basic() {
    ...
    // Start transport
    transport.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start HTTPS transport");
    
    // Verify it's running
    assert!(transport.is_running());
    
    // Clean up
    transport.stop().await.expect("Failed to stop HTTPS transport");
}
