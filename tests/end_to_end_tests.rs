...
use ratnet::prelude::*;
use ratnet::{
    nodes::{MemoryNode, DatabaseNode, FilesystemNode},
    transports::{UdpTransport, MemoryTransport},
...
async fn test_cross_node_type_communication() {
    ...
    let database = Arc::new(SqliteDatabase::new_memory().await.expect("Failed to create database"));
    let database_node = Arc::new(DatabaseNode::new(database).await.expect("Failed to create database node"));
    ...
    // Start all nodes
    memory_node.start().await.expect("Failed to start memory node");
    database_node.start().await.expect("Failed to start database node");
    filesystem_node.start().await.expect("Failed to start filesystem node");
    
    // Test MemoryNode → DatabaseNode communication
    info!("Testing MemoryNode → DatabaseNode communication");
    let memory_id = memory_node.id().await.expect("Failed to get memory node ID");
    let database_id = database_node.id().await.expect("Failed to get database node ID");
    ...
    memory_node.send_msg(test_msg).await.expect("Failed to send message");
    
    // Test DatabaseNode → FilesystemNode communication
    info!("Testing DatabaseNode → FilesystemNode communication");
    let filesystem_id = filesystem_node.id().await.expect("Failed to get filesystem node ID");
    ...
    database_node.send_msg(test_msg2).await.expect("Failed to send message");
    ...
    filesystem_node.send_msg(test_msg3).await.expect("Failed to send message");
...
async fn test_cross_transport_communication() {
    ...
    // Start transports
    udp_transport.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start UDP transport");
    ...
    node1.send_msg(test_msg).await.expect("Failed to send message via UDP");
    ...
    node2.send_msg(test_msg2).await.expect("Failed to send message via Memory");
    
    // Clean up
    udp_transport.stop().await.expect("Failed to stop UDP transport");
    node1.stop().await;
    node2.stop().await;
    node3.stop().await;
...
async fn test_multi_hop_message_routing() {
    ...
    node_a.send_msg(test_msg).await.expect("Failed to send message from A");
    ...
    node_c.send_msg(test_msg2).await.expect("Failed to send message from C");
...
async fn test_bundle_routing_through_multiple_nodes() {
    ...
    let msg1 = Msg {
        ...
        is_chan: true,
        pubkey: memory_node.id().await.expect("Failed to get memory node ID"),
        chunked: false,
        stream_header: false,
    };
    ...
    memory_node.dropoff(bundle.clone()).await.expect("Failed to dropoff bundle to memory node");
    
    // Dropoff bundle to database node
    info!("Dropping off bundle to database node");
    database_node.dropoff(bundle.clone()).await.expect("Failed to dropoff bundle to database node");
    
    // Dropoff bundle to filesystem node
    info!("Dropping off bundle to filesystem node");
    filesystem_node.dropoff(bundle).await.expect("Failed to dropoff bundle to filesystem node");
    ...
    assert!(memory_bundle.is_ok(), "Failed to pickup bundle from memory node");
    
    info!("Picking up bundle from database node");
    let database_bundle = database_node.pickup(database_id, 0, 10000, vec![]).await;
    assert!(database_bundle.is_ok(), "Failed to pickup bundle from database node");
    
    info!("Picking up bundle from filesystem node");
    let filesystem_bundle = filesystem_node.pickup(filesystem_id, 0, 10000, vec![]).await;
    assert!(filesystem_bundle.is_ok(), "Failed to pickup bundle from filesystem node");
...
async fn test_chunked_message_flow_across_nodes() {
    ...
    // Start nodes
    sender_node.start().await.expect("Failed to start sender node");
    receiver_node.start().await.expect("Failed to start receiver node");
    ...
    sender_node.send_msg(large_msg).await.expect("Failed to send large message");
...
async fn test_network_partition_recovery() {
    ...
    node1.send_msg(test_msg).await.expect("Failed to send message during partition");
    ...
    node1.send_msg(test_msg2).await.expect("Failed to send message after recovery");
