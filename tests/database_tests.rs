...
use ratnet::api::{Contact, Channel, Profile, Peer};
use ratnet::api::crypto::KeyPair; // <-- Add this import
use std::sync::Arc;
use tokio_test;

async fn setup_test_db() -> Arc<SqliteDatabase> {
    let db = SqliteDatabase::new_memory().await.expect("Failed to create test database");
...
async fn test_contact_operations() {
    ...
    // Test get contact
    let contact = db.get_contact("test_contact")
        .await
    ...
    let updated_contact = db.get_contact("test_contact")
        .await
    ...
    let deleted_contact = db.get_contact("test_contact")
        .await
...
async fn test_channel_operations() {
    ...
    // Test get channel
    let channel = db.get_channel("test_channel")
        .await
    ...
    let privkey = db.get_channel_privkey("test_channel")
        .await
    ...
    let deleted_channel = db.get_channel("test_channel")
        .await
...
async fn test_profile_operations() {
    ...
    // Test get profile
    let profile = db.get_profile("test_profile")
        .await
    ...
    let privkey = db.get_profile_privkey("test_profile")
        .await
    ...
    let updated_profile = db.get_profile("test_profile")
        .await
    ...
    let deleted_profile = db.get_profile("test_profile")
        .await
...
async fn test_peer_operations() {
    ...
    // Test add peer
    db.add_peer("test_peer".to_string(), true, "127.0.0.1:8080".to_string(), "default".to_string())
        .await
        .expect("Failed to add peer");
    
    // Test get peer
    let peer = db.get_peer("test_peer")
        .await
    ...
    db.add_peer("test_peer2".to_string(), false, "127.0.0.1:8081".to_string(), "secondary".to_string())
        .await
        .expect("Failed to add second peer");
    
    // Test get peers by group
    let default_peers = db.get_peers(Some("default")).await.expect("Failed to get default peers");
    assert_eq!(default_peers.len(), 1);
    assert_eq!(default_peers[0].name, "test_peer");
    
    let secondary_peers = db.get_peers(Some("secondary")).await.expect("Failed to get secondary peers");
    ...
    let deleted_peer = db.get_peer("test_peer")
        .await
    ...
    let remaining_peers = db.get_peers(None).await.expect("Failed to get remaining peers");
...
async fn test_config_operations() {
    ...
    // Test get config
    let value = db.get_config("test_key")
        .await
    ...
    let updated_value = db.get_config("test_key")
        .await
    ...
    let deleted_value = db.get_config("test_key")
        .await
...
async fn test_outbox_operations() {
    ...
    let messages = db.get_outbox_msgs(500).await.expect("Failed to get outbox messages");
    assert_eq!(messages.len(), 3);
    
    let messages_since_1500 = db.get_outbox_msgs(1500).await.expect("Failed to get filtered messages");
    assert_eq!(messages_since_1500.len(), 2);
    
    // Test delete old messages
    db.delete_outbox_msgs(2500).await.expect("Failed to delete old messages");
    
    let remaining_messages = db.get_outbox_msgs(0).await.expect("Failed to get remaining messages");
...
async fn test_streaming_operations() {
    ...
    // Test get stream
    let stream = db.get_stream(stream_id)
        .await
    ...
    let chunk1 = db.get_chunk(stream_id, 0)
        .await
    ...
    let all_chunks = db.get_stream_chunks(stream_id)
        .await
    ...
    let remaining_chunks = db.get_stream_chunks(stream_id)
        .await
    ...
    let deleted_stream = db.get_stream(stream_id)
        .await
...
async fn test_multiple_database_instances() {
    ...
    // Verify isolation
    let db1_contacts = db1.get_contacts().await.expect("Failed to get db1 contacts");
    let db2_contacts = db2.get_contacts().await.expect("Failed to get db2 contacts");
