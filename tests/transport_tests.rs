...
async fn test_tls_transport_with_certs() {
    ...
    let node = Arc::new(ratnet::nodes::MemoryNode::new());
    let transport = TlsTransport::new_with_certs(
...
async fn test_https_transport_with_certs() {
    ...
    let node = Arc::new(ratnet::nodes::MemoryNode::new());
    let transport = HttpsTransport::new_with_certs(
        "127.0.0.1:0".to_string(),
        cert_pem,
        key_pem,
        true,
...
async fn test_transport_json_serialization() {
    ...
    let deserialized = ratnet::transports::UdpTransport::from_json(&json).expect("Failed to deserialize transport");
...
async fn test_udp_transport_concurrent_access() {
    ...
    // Start listening
    transport.listen("127.0.0.1:0".to_string(), false).await.expect("Failed to start listening");
