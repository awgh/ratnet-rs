...
use bytes::Bytes;
use ratnet::api::{Msg, PubKey, chunking, Node};
...
async fn test_chunk_handling() {
    ...
    node.add_stream(stream_id, total_chunks, "test-channel".to_string()).await.unwrap();
    
    // Add chunks
    let chunk_data = Bytes::from("chunk data 0");
    node.add_chunk(stream_id, 0, chunk_data.clone()).await.unwrap();
    
    let chunk_data = Bytes::from("chunk data 1");
    node.add_chunk(stream_id, 1, chunk_data.clone()).await.unwrap();
    
    let chunk_data = Bytes::from("chunk data 2");
    node.add_chunk(stream_id, 2, chunk_data.clone()).await.unwrap();
...
async fn test_chunked_message_format() {
    ...
    node.add_stream(stream_id, 2, "test-channel".to_string()).await.unwrap();
